...
describe('mapUpdateError', () => {
    it('maps network errors', () => {
        expect(mapUpdateError(new Error('network timeout'))).toBe('Unable to check for updates right now.')
        expect(mapUpdateError(new Error('ECONNREFUSED'))).toBe('Unable to check for updates right now.')
        expect(mapUpdateError(new Error('fetch failed'))).toBe('Unable to check for updates right now.')
    })

    it('maps signature errors', () => {
        expect(mapUpdateError(new Error('signature verification failed'))).toBe('The update could not be verified.')
        expect(mapUpdateError(new Error('certificate invalid'))).toBe('The update could not be verified.')
    })

    it('maps download errors', () => {
        expect(mapUpdateError(new Error('download interrupted'))).toBe('The update could not be downloaded.')
        expect(mapUpdateError(new Error('transfer failed'))).toBe('The update could not be downloaded.')
    })

    it('maps install errors', () => {
        expect(mapUpdateError(new Error('install failed'))).toBe('The update could not be installed.')
        expect(mapUpdateError(new Error('extract error'))).toBe('The update could not be installed.')
    })

    it('maps disk-space errors to a friend-readable install message', () => {
        expect(mapUpdateError(new Error('no space left on device'))).toBe('The update could not be installed.')
        expect(mapUpdateError(new Error('ENOSPC'))).toBe('The update could not be installed.')
    })

    it('maps malformed metadata to a verification message', () => {
        expect(mapUpdateError(new Error('invalid signature'))).toBe('The update could not be verified.')
        expect(mapUpdateError(new Error('unexpected end of JSON input'))).toBe('An update error occurred.')
    })
