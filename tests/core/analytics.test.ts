...
describe('metricTrend', () => {
    it('computes a positive WPM trend over chronological order', () => {
        const points = [pt({ startedAt: 3, wpm: 20 }), pt({ startedAt: 1, wpm: 10 }), pt({ startedAt: 5, wpm: 30 })]
    ...
    it('trends are based on chronological order regardless of array order', () => {
        const shuffled = [pt({ startedAt: 5, wpm: 30 }), pt({ startedAt: 1, wpm: 10 }), pt({ startedAt: 3, wpm: 20 })]
