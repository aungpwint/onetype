...
describe('practice material builder', () => {
    it('builds an English timed run with fresh opaque id', () => {
        const a = buildPracticeMaterial({ language: 'english', unit: 'time', time: 30 })
        const b = buildPracticeMaterial({ language: 'english', unit: 'time', time: 30 })
    ...
    it('builds a fixed word-count English run', () => {
        const m = buildPracticeMaterial({ language: 'english', unit: 'words', words: 50 })
        const words = m.sequence.text.trim().split(/\s+/)
        expect(words).toHaveLength(50)
    })

    it('creates a custom-text run verbatim', () => {
        const text = 'the quick brown fox jumps over the lazy dog'
        const m = buildPracticeMaterial({ language: 'english', unit: 'text', text })
        expect(m.sequence.text.replaceAll(' ', ' ').startsWith(text)).toBe(true)
        expect(m.sequence.text).toContain(text)
    })

    it('builds encodable Myanmar material', () => {
        const m = buildPracticeMaterial({ language: 'myanmar', unit: 'time', time: 30 })
    ...
    it('throws for empty custom text', () => {
        expect(() => buildPracticeMaterial({ language: 'english', unit: 'text', text: '   ' })).toThrow()
    })

    it('sprinkles English punctuation and capitalisation', () => {
        const m = buildPracticeMaterial({ language: 'english', unit: 'words', words: 120, punctuation: true })
        const punctuation = /[.,!?;:]/
        const capitals = /[A-Z]/
    ...
    it('sprinkles numbers without breaking word count', () => {
        const m = buildPracticeMaterial({ language: 'english', unit: 'words', words: 60, numbers: true })
        expect(m.sequence.text.split(/\s+/)).toHaveLength(60)
        expect(m.sequence.text).toMatch(/[0-9]{1,3}/)
    })

    it('decorates Myanmar with native punctuation and numerals, all encodable', () => {
        const m = buildPracticeMaterial({ language: 'myanmar', unit: 'words', words: 150, punctuation: true, numbers: true })
        const text = m.sequence.text
        expect(text).toMatch(/[\u104A\u104B]/) // ၊ ။
    ...
    it('decorates timed runs via a word-based build', () => {
        const m = buildPracticeMaterial({ language: 'english', unit: 'time', time: 15, punctuation: true, numbers: true })
        const text = m.sequence.text
        expect(text).toMatch(/[.,!?;:]/)
    ...
    it('picks a real quotation from the curated pool', () => {
        ...
        for (const language of ['english', 'myanmar'] as const) {
            ...
            const m = buildPracticeMaterial({ language, unit: 'quote' })
    ...
    it('quote mode stays untimed (no duration bound)', () => {
        const m = buildPracticeMaterial({ language: 'english', unit: 'quote' })
