...
describe('focusCharsFromWeakFingers', () => {
    ...
    it('dedupes and caps across fingers', () => {
        const chars = focusCharsFromWeakFingers(['left-middle', 'left-index'], { maxKeys: 3 })
...
describe('reinforcementFromWeakKeys', () => {
    ...
    it('throws when nothing maps to the layout', () => {
        expect(() => reinforcementFromWeakKeys([{ key: 'F13:none', lowerBound: 0 }])).toThrow('no weak keys mapped')
    })
})
...
describe('reinforcementFromWeakFingers', () => {
    ...
    it('throws for fingers with no drillable keys', () => {
        expect(() => reinforcementFromWeakFingers(['left-thumb'])).toThrow('no weak fingers carry')
    })
})
