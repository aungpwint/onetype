...
describe('resolvePressedKey — positional code path', () => {
    it('prefers event.code when the layout knows the key', () => {
        expect(resolvePressedKey({ code: 'KeyZ', key: 'z', shiftKey: false }, myanmar)).toEqual({ code: 'KeyZ', modifier: 'none' })
        expect(resolvePressedKey({ code: 'Digit7', key: 'ရ', shiftKey: true }, myanmar)).toEqual({ code: 'Digit7', modifier: 'shift' })
        expect(resolvePressedKey({ code: 'Space', key: ' ', shiftKey: false }, myanmar)).toEqual({ code: 'Space', modifier: 'none' })
        expect(resolvePressedKey({ code: 'KeyA', key: 'a', shiftKey: false }, englishQwerty)).toEqual({ code: 'KeyA', modifier: 'none' })
    })
})
...
describe('resolvePressedKey — OS keyboard layout fallback', () => {
    it('falls back to the normalized event.key when event.code is missing or unidentified', () => {
        ...
        expect(resolvePressedKey(e, myanmar)).toEqual({ code: 'KeyA', modifier: 'none' })
        expect(normalizeMyanmarText(e.key)).toBe('\u1031')
    })

    it('reverse-maps the OS-produced Myanmar character back to its hint key', () => {
        expect(resolvePressedKey({ code: '', key: 'ရ', shiftKey: false }, myanmar)).toEqual({ code: 'Digit7', modifier: 'shift' })
        expect(resolvePressedKey({ code: '', key: 'ေ', shiftKey: false }, myanmar)).toEqual({ code: 'KeyA', modifier: 'none' })
        expect(resolvePressedKey({ code: '', key: ' ', shiftKey: false }, myanmar)).toEqual({ code: 'Space', modifier: 'none' })
    })
    ...
    it('prefers the character the learner actually produced when code and key disagree', () => {
        ...
        expect(resolvePressedKey({ code: 'KeyR', key: 'ေ', shiftKey: false }, myanmar)).toEqual({ code: 'KeyA', modifier: 'none' })
        expect(resolvePressedKey({ code: 'KeyA', key: 'ရ', shiftKey: true }, myanmar)).toEqual({ code: 'Digit7', modifier: 'shift' })
    })

    it('keeps the code path for plain keys whose character maps nowhere (e.g. dead keys)', () => {
        expect(resolvePressedKey({ code: 'KeyQ', key: 'Dead', shiftKey: false }, myanmar)).toEqual({ code: 'KeyQ', modifier: 'none' })
    })
...
describe('typing "ရေ" through OS-emitted events matches the app target exactly', () => {
    it('a Pyidaungsu-layout session emits the canonical sequence with no U+200C', () => {
        ...
        expect(keys).toEqual([
            { code: 'Digit7', modifier: 'shift' },
            { code: 'KeyA', modifier: 'none' },
        ])
        const typed = keys
            .map((k) => myanmar.outputFor(k!.code, k!.modifier)?.text ?? '')
            .join('')
