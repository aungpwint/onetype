...
function Metric({ label, value, tone }: { label: string; value: string; tone?: 'success' | 'destructive' | 'muted' }) {
    const color = tone === 'success' ? 'text-success' : tone === 'destructive' ? 'text-destructive' : 'text-foreground'
    return (
        <div className="flex flex-col items-center gap-0.5">
            <span className={`tnum text-lg leading-none font-semibold md:text-xl ${color}`}>{value}</span>
            <span className="text-[0.6875rem] font-medium tracking-[0.12em] text-muted-foreground uppercase">{label}</span>
        </div>
    )
}
    ...
    return (
        <div className="flex w-full flex-col items-center">
            <div className="flex items-baseline justify-center gap-3">
                <span className="tnum text-6xl leading-none font-semibold tracking-tight text-foreground sm:text-7xl">{wpm}</span>
                <span className="text-sm font-semibold tracking-[0.18em] text-muted-foreground uppercase">WPM</span>
            </div>
            ...
            <div className="mt-3 flex items-center gap-5">
                ...
                />
                <span className="h-4 w-px bg-line-strong/60" aria-hidden />
                <Metric label="Errors" value={String(stats.incorrectCount)} tone={stats.incorrectCount > 0 ? 'destructive' : 'muted'} />
            </div>
        </div>
    )
}
