...
export function envelopeP(
  state: FingerAnimState,
  prof: FingerMotionProfile,
  now: number,
  reduced: boolean,
): number {
 ...
 */
export function toTransformAttribute(
  geo: FingerGeometry,
  prof: FingerMotionProfile,
  target: FingerTarget | null,
  p: number,
...
export class FingerAnimator {
  ...
  setTarget(finger: FingerId, target: FingerTarget | null, now = performance.now()): void {
    ...
    const entryP = prev?.target !== undefined && prev.target !== null
      ? envelopeP(prev, prof, now, this.reduced)
      : 0;
