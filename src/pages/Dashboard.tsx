      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="card p-5">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            ...
            <span className="text-sm text-ink-soft">days{streak?.current === 1 ? "" : "s"}</span>
            {streak && streak.current > 0 ? <span aria-hidden className="text-lg">🔥</span> : null}
          </div>
        ...
        <div className="card p-5">
          ...
          <dl className="mt-3 space-y-1.5 text-sm">
            <div className="flex justify-between"><dt className="text-ink-soft">Fastest WPM</dt><dd className="tnum">{stats.bestWpm ? Math.round(stats.bestWpm) : "—"}</dd></div>
            <div className="flex justify-between"><dt className="text-ink-soft">Typing time</dt><dd className="tnum">{summary ? formatDuration(summary.totalMinutes * 60000) : "—"}</dd></div>
            <div className="flex justify-between"><dt className="text-ink-soft">Sessions</dt><dd className="tnum">{summary?.sessions ?? "—"}</dd></div>
          </dl>
        </div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="card overflow-hidden">
          <div className="flex items-center justify-between border-b border-line px-5 py-3">
            ...
            <Link to="/progress" className="text-sm text-accent hover:underline">Chart →</Link>
          </div>
          <div className="p-5">
            ...
            {Object.entries(ACHIEVEMENT_CATALOG).map(([id, def]) => {
              ...
              return (
                <span
                  ...
                  className={`flex h-10 w-10 items-center justify-center rounded-lg border text-lg transition-colors ${earned ? "border-transparent" : "opacity-35 grayscale"}`}
          ...
          <div className="mt-5 border-t border-line pt-4">
            ...
            <ul className="mt-2 grid gap-1.5 sm:grid-cols-2">
              {languageStats.length === 0 ? (
                <li className="text-sm text-ink-faint">No completed sessions yet.</li>
              ) : (
                languageStats.map((l) => (
                  <li key={l.lang} className="flex items-center justify-between text-sm">
                    <span className="capitalize text-ink-soft">{l.lang === "mixed" ? "Mixed" : l.lang === "myanmar" ? "Myanmar" : "English"}</span>
                    <span className="tnum text-ink-faint">{l.sessions} runs · {l.avgWpm} wpm</span>
                  </li>
                ))
              )}
            </ul>
