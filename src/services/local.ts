...
export const localBackend = {
    ...
    nextExerciseAttempt: async (studentId: string, exerciseId: string): Promise<number> => {
        const results = read<ExerciseResult[]>(KEYS.exerciseResults, []).filter((r) => r.exerciseId === exerciseId && r.studentId === studentId)
...
function evaluateLegacyAchievements(studentId: string): AchievementRecord[] {
    ...
    const lessonPassed = read<LessonProgress[]>(KEYS.lessonProgress, []).some((p) => p.studentId === studentId && p.completed)
