...
export function buildSequence(
  ...
  const graphemes =
    layout.language === "myanmar"
      ? splitMyanmarSyllables(text)
      : splitGraphemes(text);
  ...
  for (let gi = 0; gi < graphemes.length; gi++) {
    ...
    for (const pair of pairs) {
      ...
      units.push({
        ...
        shiftHand:
          pair.lookup.modifier === "shift"
            ? shiftHandFor(pair.lookup.hand)
            : null,
        graphemeIndex: gi,
        grapheme: token,
      });
...
export function graphemeForUnit(
...
export function clusterStartForUnit(
  sequence: BuiltSequence,
  unitIndex: number,
): number {
  if (unitIndex <= 0) return 0;
...
export function remainingText(
  ...
  return sequence.graphemes
    .slice(graphemeForUnit(sequence, unitIndex))
    .join("");
}
...
function unitsForGraphemesBefore(
  sequence: BuiltSequence,
  unitIndex: number,
): number {
