...
- **App is stuck on "preparing" or won't open a lesson/test/drill?** OneType retries automatically a few times, then shows a _Try again_ button — if the message keeps coming back, close and reopen the app, then check the release notes in case the issue was already fixed.
