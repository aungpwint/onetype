...
describe('generate-latest-json.mjs (partial)', () => {
    it('writes a partial manifest with the platform key and verbatim signature', () => {
        ...
        run('--partial', '--target', 'windows-x86_64', '--url', 'https://example.com/dummy-setup.exe', '--sig-file', sigFile, '--out', out)
    ...
    it('rejects unknown platform keys', () => {
        ...
        expect(() => run('--partial', '--target', 'windows-commodore64', '--url', 'u', '--sig-file', sigFile, '--out', out)).toThrow()
    })

    it('fails when the signature file is missing', () => {
        const out = join(dir, 'missing.json')
        expect(() => run('--partial', '--target', 'windows-x86_64', '--url', 'u', '--sig-file', join(dir, 'nope.sig'), '--out', out)).toThrow(
            /not found/i,
        )
    })
})
...
describe('generate-latest-json.mjs (merge)', () => {
    it('merges multiple partials into one manifest', () => {
        ...
        run('--merge', '--version', '1.1.0', '--pub-date', '2026-01-01T00:00:00Z', '--notes', 'Notes', '--partials', `${p1} ${p2}`, '--out', out)
        const m = JSON.parse(readFileSync(out, 'utf8'))
        expect(m.version).toBe('1.1.0')
        expect(m.notes).toBe('Notes')
        expect(Object.keys(m.platforms)).toEqual(expect.arrayContaining(['windows-x86_64', 'darwin-aarch64']))
    })
    ...
    it('rejects duplicate platforms', () => {
        ...
        expect(() => run('--merge', '--version', '1.1.0', '--pub-date', 'x', '--partials', `${p} ${p}`, '--out', out)).toThrow(/duplicate platform/i)
    })

    it('rejects an empty merge', () => {
        const out = join(dir, 'empty.json')
        expect(() => run('--merge', '--version', '1.1.0', '--pub-date', 'x', '--partials', '', '--out', out)).toThrow()
    })
    ...
    it('ignores whitespace padding around the partials list', () => {
        ...
        run('--merge', '--version', '1.1.0', '--pub-date', 'x', '--partials', `  ${p1}\n  `, '--out', out)
        const m = JSON.parse(readFileSync(out, 'utf8'))
    ...
    it('reports a clear error when a partial path is a directory', () => {
        ...
        expect(() => run('--merge', '--version', '1.1.0', '--pub-date', 'x', '--partials', dir, '--out', out)).toThrow(/not a file/i)
    })

    it('reports a clear error when a partial is missing', () => {
        const out = join(dir, 'missing-out.json')
        expect(() => run('--merge', '--version', '1.1.0', '--pub-date', 'x', '--partials', join(dir, 'nope.json'), '--out', out)).toThrow(
            /not found/i,
        )
    })
    ...
    it('reports a clear error on invalid JSON in a partial', () => {
        ...
        expect(() => run('--merge', '--version', '1.1.0', '--pub-date', 'x', '--partials', bad, '--out', out)).toThrow(/invalid json/i)
    })
})
