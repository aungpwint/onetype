...
function categorisedLog(previousTag, currentTag) {
    ...
    try {
        const log = execFileSync('git', ['log', '--oneline', '--no-merges', `${previousTag}..${currentTag}`], {
            cwd: root,
            encoding: 'utf8',
...
function buildNotes() {
    ...
    const checksumsAbsolute = checksumsPath ? (isAbsolute(checksumsPath) ? checksumsPath : resolve(root, checksumsPath)) : null
    const checksumsText = checksumsAbsolute && existsSync(checksumsAbsolute) ? readFileSync(checksumsAbsolute, 'utf8').trim() : ''
    ...
    try {
        const tagged = execFileSync('git', ['tag', '-l', 'v*', '--sort=-version:refname'], {
            cwd: root,
            encoding: 'utf8',
