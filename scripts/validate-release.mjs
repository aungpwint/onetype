...
const DISTRIBUTABLE_EXTENSIONS = new Set(['.exe', '.msi', '.dmg', '.app.tar.gz', '.AppImage', '.deb', '.rpm'])
...
function validateRelease(options) {
    ...
    if (manifest.version !== version) {
        fail(`latest.json version "${manifest.version}" does not match released version "${version}".`)
    }
...
function run() {
    ...
    if (!version) {
        ...
        try {
            version = JSON.parse(readFileSync(resolve(process.cwd(), 'package.json'), 'utf8')).version
        } catch {
