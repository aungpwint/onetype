...
writeFileSync(pkgPath, pkg.replace(/("version"\s*:\s*")[^"]+(")/, `$1${next}$2`))

// Cargo.toml — rewrite [package] version line, preserving everything else.
const cargoPath = resolve(root, 'src-tauri/Cargo.toml')
writeFileSync(cargoPath, readFileSync(cargoPath, 'utf8').replace(/^version\s*=\s*"[^"]+"/m, `version = "${next}"`))

// tauri.conf.json — swap top-level version, preserving formatting.
const confPath = resolve(root, 'src-tauri/tauri.conf.json')
const conf = readFileSync(confPath, 'utf8')
writeFileSync(confPath, conf.replace(/(^\s*"version"\s*:\s*")[^"]+(")/m, `$1${next}$2`))
