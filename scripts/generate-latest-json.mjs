...
const PLATFORM_KEYS = new Set(['windows-x86_64', 'windows-aarch64', 'darwin-x86_64', 'darwin-aarch64', 'linux-x86_64', 'linux-aarch64'])
...
function modeMerge(version, pubDate, notes, partialGlob, out) {
    ...
    const partialPaths = partialGlob ? partialGlob.split(argsDelimiter()).filter(Boolean) : []
...
const mode = process.argv.includes('--partial') ? 'partial' : process.argv.includes('--merge') ? 'merge' : null
