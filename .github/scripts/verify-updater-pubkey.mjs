...
if (sigId.equals(configId)) {
    console.log(`[verify-updater-pubkey] OK — signing key matches plugins.updater.pubkey (${comment ?? keyId.toString('hex').toUpperCase()}).`)
    process.exit(0)
}

fail(
    `Signing key mismatch. The .sig carries key id ${bytesToHex(sigId)}, but plugins.updater.pubkey embeds ${bytesToHex(
        configId,
    )}. Update src-tauri/tauri.conf.json (or the CI signing secrets) so they agree, ` + 'otherwise installed apps will reject every update.',
)
